...
- **AI:** OpenRouter (REST API tương thích OpenAI, gọi qua `RestClient` của Spring — không dùng SDK riêng của nhà cung cấp) — dùng cho AI Coach chat, tóm tắt hội thoại, và trích xuất bộ nhớ tự động.
...
| **Trò chuyện AI Coach** | `POST /api/conversations` | Bắt đầu phiên chat mới (**bị giới hạn quota** — mặc định tối đa 5 phiên/người dùng, kiểm soát chi phí gọi model) |
...
1. **Chat thời gian thực (`CoachAgentService`):** khi người dùng gửi tin nhắn, hệ thống gọi model qua OpenRouter (mặc định `~deepseek/deepseek-flash-latest`) với một system prompt tiếng Việt quy định vai trò "huấn luyện Socratic" — phản chiếu câu hỏi lại cho người dùng tự suy ngẫm, **không chẩn đoán, không trị liệu, nếu phát hiện khủng hoảng thì hướng dẫn tìm chuyên gia** — cá nhân hoá theo Core Values của người dùng.
2. **Kết thúc phiên → trích xuất bộ nhớ (`MemoryExtractionService` + `MemoryExtractionPersister`):** khi người dùng bấm "Kết thúc phiên", một sự kiện được phát ra sau khi transaction commit thành công, xử lý **bất đồng bộ** (thread pool riêng `memoryExtractionExecutor`): gọi model qua OpenRouter ở chế độ trả về JSON (`response_format: json_object`, cấu trúc mong muốn được mô tả ngay trong prompt) để tự động trích xuất ra: người được nhắc tới (Person), sự kiện mối quan hệ (RelationshipEvent), và insight (Insight) — rồi ghi vào database. Có thể tuỳ chọn xoá nội dung tin nhắn gốc sau khi trích xuất xong (data minimization).
3. **Tóm tắt theo yêu cầu (`ConversationSummaryService`):** sinh bản tóm tắt markdown cho 1 phiên, dùng cùng model rẻ với bước trích xuất bộ nhớ, có thể gọi lại nhiều lần để làm mới.
4. **Đọc tâm trạng (`MoodScoringService`) — KHÔNG dùng AI:** đây là **heuristic đối chiếu từ khoá chạy cục bộ trong ứng dụng**, **không gọi OpenRouter/LLM** (khác với 3 mục trên) — một từ điển nhỏ Anh + Việt cho 9 cảm xúc, mỗi cảm xúc có "độ nặng" 0–1; văn bản được chuyển chữ thường rồi dò chuỗi con, nhiều từ khoá khớp thì lấy cảm xúc **nặng nhất**. Khi người dùng gửi tin nhắn, kết quả được lưu vào chính tin nhắn đó (`mood_emotion/mood_score`); khi phiên kết thúc, tin nhắn USER **đầu tiên và cuối cùng** có điểm sẽ được chốt thành cung cảm xúc của phiên (`initial_*`/`final_*`) — nếu không tin nhắn nào khớp thì cả 4 trường để trống. Kết thúc phiên lần thứ hai không tính lại (giữ nguyên tính idempotent nhờ nhánh chỉ-chạy-khi-ACTIVE có sẵn). Cùng bảng độ nặng này được `UserStatsService` dùng lại để chấm cảm xúc của entry khi dựng `mood-summary`.
...
- **Chống lạm dụng:** giới hạn tần suất theo IP cho các route auth, giới hạn theo người dùng cho việc gửi tin nhắn chat, và hạn mức cứng số phiên chat tối đa/người dùng (mặc định 5) — mục đích kiểm soát chi phí gọi model trong giai đoạn đầu ra mắt.
...
| **OpenRouter** (mặc định model `~deepseek/deepseek-flash-latest`, khai báo ở `AiConstants`) | AI Coach chat, tóm tắt hội thoại, trích xuất bộ nhớ | Khoá API qua `OPENROUTER_API_KEY`. Gọi thẳng endpoint `POST /chat/completions` (tương thích OpenAI) bằng `RestClient`, **không streaming** — mỗi lần gọi trả về một câu trả lời hoàn chỉnh. Có thể đổi endpoint/timeout/model qua `OPENROUTER_BASE_URL`, `OPENROUTER_TIMEOUT_SECONDS` (mặc định 60s). Nếu thiếu khoá, app vẫn khởi động bình thường và chỉ báo lỗi ở lần gọi model đầu tiên. |
...
- Không có test tự động cho controller/repository (chỉ có unit test ở tầng service và cho `OpenRouterClient`); CI hiện tại chạy `mvn clean verify -DskipTests` nên **không có test nào thực sự chạy** trong pipeline.
...
| 2026-09-17 | Claude (agent) | Chuyển toàn bộ tầng AI Coach từ Google Gemini (SDK `google-genai`) sang **OpenRouter** qua REST API tương thích OpenAI (`ai/OpenRouterClient`, dùng `RestClient`, không streaming): áp dụng cho chat (`CoachAgentService`), tóm tắt (`ConversationSummaryService`) và trích xuất bộ nhớ (`MemoryExtractionService` — schema JSON nay mô tả trong prompt + `response_format: json_object` thay cho `responseSchema` của Gemini). Biến môi trường `GEMINI_API_KEY` được thay bằng `OPENROUTER_API_KEY` (cả trong workflow deploy); gỡ dependency `google-genai` cùng cặp `jackson-module-kotlin`/`kotlin-reflect` vốn chỉ tồn tại để vá lỗi do jar đó gây ra. Không đổi endpoint hay response shape nào — `reflectly-fe` không cần sửa gì ngoài việc nới timeout cho 2 lời gọi có chạy model. |
